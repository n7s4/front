        ...
        <ul class="cart-list" v-for="cart in cartInfoList" :key="cart.id">
          ...
          <li class="cart-list-con5">
            ...
            <input 
              ...
              @click="handle('change',$event.target.value*1,cart)"
              class="itxt"
            >
  ...
  export default {
    ...
    methods:{
      ...
      async handle(type,skuNum,cart){
        ...
        try {
          //代表的是修改成功
          await this.$store.dispatch('addOrUpdateShopCart',{skuId:cart.id,skuNum:skuNum})
          //再一次获取服务器最新的数据进行展示
          this.getData()
        } catch (error) {
    ...
    computed:{
      ...
      isCartChecked(){
        ...
        return this.cartInfoList.filter(item=>item.isCartChecked=='1').length === this.cartInfoList.length;
      },
    }
  }
</script>
